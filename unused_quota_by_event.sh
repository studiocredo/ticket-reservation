...
select distinct(u.email) from "quota" q left join "prereservations" p on q.user_id = p.user_id join "user_detail" u on q.user_id = u.id where p.quantity is null) To STDOUT With CSV DELIMITER ',';
