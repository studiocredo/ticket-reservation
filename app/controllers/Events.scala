...
object Events extends Controller {
  ...
  def create() = Action { implicit request =>
    Ok(views.html.eventsCreateForm(eventForm))
  }
  def save() = DBAction { implicit rs =>
    eventForm.bindFromRequest.fold(
      formWithErrors => BadRequest(views.html.eventsCreateForm(formWithErrors)),
      event => {
  ...
  def edit(id: EventId) = DBAction { implicit rs =>
    eventService.getEdit(id) match {
      case None => ListPage
      case Some(event) => Ok(views.html.eventsEditForm(id, eventForm.fillAndValidate(event)))
    }
  }
  def update(id: EventId) = DBAction { implicit rs =>
    eventForm.bindFromRequest.fold(
      formWithErrors => BadRequest(views.html.eventsEditForm(id, formWithErrors)),
      event => {
