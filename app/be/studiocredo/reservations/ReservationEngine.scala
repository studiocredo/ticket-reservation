...
import be.studiocredo.reservations.SeatState.{FloorplanState, SeatState}
import be.studiocredo.util.Money
import com.google.inject.Inject
import controllers.auth.Mailer
import models.entities
import models.entities.SeatType._
import models.entities.{FloorPlan, Row, Seat, SeatId, SeatWithStatus, ShowAvailability, Spacer, _}
import models.ids.{OrderId, ShowId, UserId}
...
case class SuggestionScore(partialScore: Int, separationScore: Int, orphanScore: Int, missingSeatTypeScore: Int) {
  val value: Int = partialScore + separationScore + orphanScore + missingSeatTypeScore
...
case class SeatSuggestion(partials: List[PartialSeatSuggestion], initialSeats: AvailableSeats, availableTypes: Set[SeatType]) {
  ...
  val seatIds: List[SeatId] = partials.flatMap(_.seatIds)
  val seats: List[RowContent] = initialSeats.getWithSelected(seatIds)

  val score: SuggestionScore = {
...
case class AvailableSeats(fp: FloorPlan, availabilityByType: Map[SeatType, Int]) {
  ...
  def totalAvailable(types: Set[SeatType]): Int = {
    types.map(availabilityByType.getOrElse(_, 0)).sum
  }
...
object ReservationEngine {
    ...
    try {
      ...
      if (avail >= quantity) {
        ...
        calculateBestSuggestion(groupSizesList.head.map(getSolutionForSize), seats, availableTypes) match {
          ...
          case None =>
            groupSizesList.drop(1).flatMap(sizes => calculateBestSuggestion(sizes.map(getSolutionForSize), seats, availableTypes)) match {
  ...
  private def toSeatSuggestion(maybeSg: Option[SeatSuggestion], seats: AvailableSeats, availableTypes: Set[SeatType]) = (psg: PartialSeatSuggestion) => {
    maybeSg match {
  ...
  private def calculateAllAdjacentSuggestions(seats: AvailableSeats, sizes: List[Int]): Map[Int, List[PartialSeatSuggestion]] = {
    ...
    result.map{ case (key, value) => (key, value.toList) }
  }
}
...
class ReservationEngineMonitorService @Inject()(showService: ShowService, venueService: VenueService, orderService: OrderService, preReservationService: PreReservationService, userService: UserService) extends Service {
      ...
      Akka.system.scheduler.schedule(1.hour, 6.hours) {
        ...
        DB.withSession {
          implicit session: Session =>
            orderService.closeStale().foreach {order =>
  ...
  override def onStop() {
    ...
    seatOrderActor.foreach(Akka.system.stop)
    seatOrderActor = None
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case action: FloorAction =>
      if (!activeShows.contains(action.show))
        sender ! Status.Failure(ShowNotAcceptingOrdersException(action.show))
...
case class OrderInfo(orderId: OrderId, availableTypes: Set[SeatType], users: List[UserId], price: Money) {
  def isOrder(id: OrderId): Boolean = id == this.orderId
  def isAvailable(seatType: SeatType): Boolean = availableTypes.contains(seatType)


  val TIMEOUT = Duration(3, TimeUnit.MINUTES)
  var timeout: Long = newTimeout
  def touch(): Unit =  this.timeout = newTimeout

  def newTimeout: Long = System.currentTimeMillis() + TIMEOUT.toMillis
  def isTimedOut(current: Long): Boolean = this.timeout < current
}


object MaitreDActor {
  val MAGIC_SEQ_START: Long = System.nanoTime()
  // has to be static
...
class MaitreDActor(showId: ShowId, showService: ShowService, venueService: VenueService, orderService: OrderService, preReservationService: PreReservationService) extends Actor {
  ...
  var seq: Long = System.nanoTime() - MaitreDActor.MAGIC_SEQ_START  // stay within js range
  val orderInfoMap: mutable.Map[OrderId, OrderInfo] = mutable.Map[OrderId, OrderInfo]()
  ...
  override def receive: PartialFunction[Any, Unit] = {
    case ReloadState(show) => context.become(running(loadState(show).get), discardOld = true)
    case TimeOut =>
    case other => sender ! Status.Failure(NotInitializedException(showId))
  }

  def running(state: FloorplanState): Receive = {
    def respond(orderId: OrderId, body: (OrderInfo => List[Message])): Unit = {
      orderInfoMap.get(orderId).fold(
        sender ! Status.Failure(MissingOrderException(orderId))
      )(
          info => {
    ...
    def deductUnusedPreReservations(seatTypes: List[SeatType.Value], avail: mutable.Map[SeatType, Int], quantity: Int): Unit = {
...

    {
      case StartOrder(show, order, seats, users, price, availableTypes) =>
        ...
        if (totalAvailable < seats) {
          sender ! Status.Failure(CapacityExceededException(show, order, totalAvailable))
        } else {
          val suggested = ReservationEngine.suggestSeats(seats, myAvailable, info.availableTypes)

          sender ! suggested.fold(msg => {
            Status.Failure(FailedToAssignInitialSeatingException(msg))
          }, newSeats => {
      ...
      case ReloadState(show) =>
        sender ! loadState(showId).fold(
          Status.Failure(ReloadFailedException(showId)): Status
        )(newState => {
          newState.drainPending(state)

          context.become(running(newState), discardOld = true)
          Status.Success(null)
        })
...
object SeatState {
  ...
  class SeatState(val seat: Seat, var status: Status) extends RowContentState {
    ...
    def seatId: SeatId = seat.id

    def setReserved(userId: Option[UserId]): Unit = status = Reserved(userId)
    def setPending(info: OrderInfo): Unit = status = Pending(info)
    def setFree(): Unit = status = Free
    ...
    def isReserved: Boolean = status match {
      case reserved: Reserved => true
      case _ => false
    }
    def isReserved(userId: UserId): Boolean = status match {
  ...
  class FloorplanState(rows: List[RowState], val bySeat: Map[SeatId, SeatState]) {
    val seatCountbyType: Map[SeatType, Int] = bySeat.values.groupBy(_.kind).mapValues(_.size)

    val seatList: List[RowContentState] = rows.foldLeft(Nil: List[RowContentState])(_ ++ List(new UnhandledState(Spacer(100))) ++ _.content )

    def findSeats(info: OrderInfo): List[SeatState] =  bySeat.values.filter(_.isPending(info.orderId)).toList

    def remove(id: OrderId): Unit = {
      bySeat.values.foreach(_.remove(id))
    }
  ...
  def create(floorPlan: FloorPlan): FloorplanState = {
